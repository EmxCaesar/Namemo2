    ...
    timer(new HeapTimer()), epoller(new Epoller())
{
    ...
    //初始化info
    if(openLog) {    
        if(shutdown) { printf("========== Server init error!==========\n"); }
        else {
            printf("========== Server init ==========\n");
            printf("Port:%d, OpenLinger: %s\n", port, optLinger? "true":"false");
            printf("Listen Mode: %s, OpenConn Mode: %s\n",
                            (listenEvent & EPOLLET ? "ET": "LT"),
                            (connEvent & EPOLLET ? "ET": "LT"));
            printf("LogSys level: %d\n", logLevel);
            printf("srcDir: %s\n", HttpConnect::srcDir);
            printf("SqlConnPool num: %d, ThreadPool num: %d\n", connPoolNum, threadNum);
            printf("\n");
        }
    }
}
